Ключевая идея: ты делаешь **общее ядро**, которое потом можно форкать и превращать в разные продукты:
...
11. Поддерживаются модели prompt_guard и text_mood.
...
**Universal ML Classification Service Platform** — это универсальная backend-платформа для упаковки различных ML-моделей классификации текста в готовые продуктовые сервисы. Платформа предоставляет общее ядро: регистрацию и авторизацию пользователей, REST API, PostgreSQL, Redis/Celery для асинхронного inference, внутреннюю валюту, биллинг с резервированием кредитов, промокоды, уровни лояльности, историю запросов, Streamlit dashboard, Prometheus/Grafana, Docker Compose и тестирование с покрытием не ниже 70%. Конкретные ML-продукты подключаются через единый интерфейс `BaseClassifier` и реестр моделей `ModelRegistry`.

В базовой поставке реализуются два продукта: **SecurePrompt Guard** для определения вредоносных промптов, prompt injection и harmful-запросов, а также **TextMood Analytics** для анализа тональности, настроения и коммуникационного стиля клиентских сообщений. Архитектура позволяет форкнуть проект, заменить ML-модель и конфигурацию, после чего получить отдельный готовый ML-сервис без переписывания ядра приложения.
